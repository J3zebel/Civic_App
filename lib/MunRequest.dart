...
class MunicipalityRequestService extends StatefulWidget {
  final String muncipality;
  const MunicipalityRequestService({super.key, required this.muncipality});

  @override
  State<MunicipalityRequestService> createState() =>
      _MunicipalityRequestServiceState();
}

class _MunicipalityRequestServiceState
    extends State<MunicipalityRequestService> {
  ...
  Future<void> _fetchRequestTypes() async {
    ...
    try {
      final response =
          await supabase.from("Admin_tbl_municipalityrequesttype").select();
      setState(() {
        reqtype = response;
        isLoading = false;
      });
    ...
    try {
      ...
      if (userid != null) {
        ...
        await supabase.from('User_tbl_request').insert({
          ...
          'user_id': userid,
          'muncipality_id': widget.muncipality,
          'request_date': DateTime.now().toIso8601String(),
          'request_photo': photoUrl,
          'municipalityrequesttype_id': _selectedServiceType,
        });
