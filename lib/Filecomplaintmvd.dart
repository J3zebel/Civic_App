...
class MVDComplaintPage extends StatefulWidget {
  final String selectedMvd;

  const MVDComplaintPage({super.key, required this.selectedMvd});

  @override
  _MVDComplaintPageState createState() => _MVDComplaintPageState();
}

class _MVDComplaintPageState extends State<MVDComplaintPage> {
  ...
  Future<void> _fetchComType() async {
    ...
    try {
      final response =
          await supabase.from("Admin_tbl_mvdcomplainttype").select();
      setState(() {
        comptype = response;
        isLoading = false;
      });
